import { useContext, useEffect, useRef, useLayoutEffect, memo } from "react";
...
const AboutUs = memo(() => {
  ...
  useLayoutEffect(() => {    
    ...
    gsap.set(".pCharSpan", { filter: "contrast(0)" });
    gsap.set(".dark-pCharSpan", { filter: "contrast(0)" });
    setTimeout(() => {
      ScrollTrigger.refresh();
    }, 200);
    try {
      ...
      gsap.fromTo(
        ...
        },
        {
          ...
          stagger: 0.2,
          scrollTrigger: {
            trigger: secondPRef.current, start: "top 90%", end: "bottom 70%", scrub: 1, invalidateOnRefresh: true, immediateRender: true,
            // markers: true
          },
        }
      );
  ...
  return (
    <section>
      <div className="dark:text-white text-black">
        ...
        <div className="py-6">
          <p className="sm:w-[75%] lg:w-[70%] xl:w-[65%] 2xl:w-[60%] p-[1.25rem] mx-auto text-[0.9375rem]">
            We Mannu Dhaba & Family Restaurant are located on Khajoori Sadak, Indore-Bhopal Highway, Madhya Pradesh 462030. The average rating of our Dhaba is
            4.6 out of 5 stars. And the street address is
            <a target="_blank" className="underline font-medium" href="https://maps.app.goo.gl/q7Kjxds8koCrG4dg6"> 67W2+3C
            </a>
          ...
          <div className="sm:w-[75%] lg:w-[70%] xl:w-[65%] 2xl:w-[60%] mx-auto mb-14 p-[1.25rem] text-[1.0325rem] sm:text-[1.0625rem]">
            <h2 className="text-[1.1925em] sm:text-xl capitalize mb-4"> More about Mannu Dhaba
            </h2>
            <h2 className="mb-2 text-[1.0725em] sm:text-[1.1325em] font-medium"> Best Pure Veg Restaurant on the Indore-Bhopal Highway.
            </h2>
            {/* text-[rgb(96_96_96_/_38%)] dark:text-[#606060] */}
            <p ref={secondPRef} className="mb-2 text-[.9625em] sm:text-[1.0325em] font-normal"
            > {"Welcome to Mannu Dhaba & Family Restaurant – A Vegetarian Destination. Since 2000 , We have been serving pure vegetarian meals in a warm and family-friendly atmoshphere at Khajuri Sadak on the Indore-Bhopal Road. Experience over two decades of serving flavorful dishes crafted to satisfy every taste, offering quality and value for everyone to enjoy. Conveniently located at Khajuri Sadak on the Indore-Bhopal Road, our restaurant is easy to find on the map. For any kind of assistance or questions, feel free to call or message us directly on these numbers. ".split(" ").map((char, charind) => (<span key={charind}><span
              className={`${mode === "dark" ? "dark-pCharSpan" : "pCharSpan"} ${char === "pure" || char === "Vegetarian" || char === "vegetarian" || char === "family-friendly" || char === "year" || char === "2000" || char === "atmoshphere" ? "font-medium" : ""}`}>{`${char} `}</span>{char === "2000" ? <span>&#128519;</span> : ""}{char === "on" && charind > 80 ? <span>&#128071; </span> : ""}     </span>))}
            </p>
            <div className="flex flex-wrap gap-x-5 gap-y-3 justify-evenly text-[.9625em] sm:text-[1.0325em]"> {[
              ...
              },
              {
                ...
                key={index} classes="flex gap-2 items-center font-normal" number={numberSet.number} msg="Hello! Mannu Dhaba and Family Restaurant.">{numberSet.icon}{numberSet.number}</WhatsAppLink>) : (<button
                  key={index} title={numberSet.title} className="flex gap-2 items-center font-normal" onClick={copyNumber}>{numberSet.icon} {numberSet.number}</button>))}
            </div>
          </div>
          <ReasonAndtiming />
        </div>

...
function aboutUs() {
  ...
  const { value, setfixedMsg, PageHeading, WhatsAppLink } = useContext(globalContext)
  let render = useRef(0)
  useEffect(() => {
          ...
          <div className='flex flex-wrap gap-x-5 justify-evenly'>
            {
              [
                ...
                numberSet.title.includes('whatsApp') ?
                  <WhatsAppLink key={index} classes='flex gap-2 items-center' number={numberSet.number} msg='Hello! Mannu Dhaba and Family Restaurant.'>
                    {numberSet.icon }
                    {numberSet.number}
                  </WhatsAppLink> :
                  <button key={index} title={numberSet.title} className='flex gap-2 items-center' onClick={copyNumber}>{numberSet.icon} {numberSet.number}</button>

...
function aboutUs() {
  ...
  const { value, notificationMsgs, NotificationTimings, setnotificationState, PageHeading, WhatsAppLink } =
    ...
    <section>
      <div className="dark:text-white text-black backdrop-blur-[100px]">
        ...
        <div className="mb-4 p-[1.25rem]">
          <h2 className="text-[1.1925rem] sm:text-xl capitalize mb-4"> More about Mannu Dhaba
          </h2>
          <h2 className="mb-2 sm:w-[75%] mx-auto text-[1.1025rem] sm:text-[1.1325rem] font-semibold"> Best Pure Veg Restaurant on the Indore-Bhopal Highway.
          </h2>
          <p ref={secondPRef} className="mb-2 sm:w-[75%] mx-auto text-[1.0325rem] sm:text-[1.0625rem]"
          > {"Welcome to Mannu Dhaba & Family Restaurant – A Vegetarian Destination. Since 2000 , We have been serving pure vegetarian meals in a warm and family-friendly atmoshphere at Khajuri Sadak on the Indore-Bhopal Road. Experience over two decades of serving flavorful dishes crafted to satisfy every taste, offering quality and value for everyone to enjoy. Conveniently located at Khajuri Sadak on the Indore-Bhopal Road, our restaurant is easy to find on the map. For any kind of assistance or questions, feel free to call or message us directly on these numbers. ".split(" ").map((char, charind) => (<span key={charind}><span
            className={`${value === "dark" ? "dark-pCharSpan dark:text-[#606060]" : "pCharSpan text-[rgb(96_96_96_/_38%)]"} ${char === "pure" || char === "Vegetarian" || char === "vegetarian" || char === "family-friendly" || char === "year" || char === "2000" || char === "atmoshphere" ? "font-bold" : ""}`}>{`${char} `}</span>{char === "2000" ? <span>&#128519;</span> : ""}{char === "on" && charind > 80 ? <span>&#128071; </span> : ""}     </span>))}
          </p>
          <div className="flex flex-wrap gap-x-5 gap-y-4 justify-evenly"> {[
            {

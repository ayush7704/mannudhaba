...
let Menulink = memo(({ value, reff }) => {
  ...
  let btnmouse = contextSafe((event) => {
    gsap.to(reff.current, { background: `-webkit-linear-gradient(${event.nativeEvent.offsetX}deg, hsla(43, 84%, 85%, 1) 0%, hsla(325, 71%, 70%, 1) 30%, hsla(236, 67%, 55%, 1) 100%)`, duration: 0.23 })
  })

  return (
    <Link to="/menu" ref={reff} onMouseMove={btnmouse} className={`svgbtn outLine linear-btn font-medium tracking-[0.5px] py-[0.625rem] text-[1rem] text-center rounded-[6px] text-white px-[1.1875rem] min-w-[50%] text-nowrap flex gap-2 justify-center items-center shadow-xl hover:shadow-2xl [textShadow:0.125rem_1px_3px_#3b3b3b94]`}>
      <span>{value}</span>
...
const ReasonAndtiming = memo(() => {
  return (
    <section className='p-[1.25rem]'>
      <h3 className='capitalize font-medium max-sm:text-[1.15rem] text-xl pb-3 text-center tracking-[0.7px] [text-shadow:2px_2px_0px_#00000035]'>why & when you should visit ?</h3>

      <div className="reason sm:w-[75%] lg:w-[60%] xl:w-[55%] mx-auto pb-3 max-sm:text-[.95rem] text-[1rem]">
        <h3 className='capitalize font-normal text-lg px-3 pb-6 text-center [textShadow:0.125rem_1px_3px_#3b3b3b94]'>we are open everyday from <strong className='inline-block tracking-[1px]'>&nbsp;10AM - 2AM</strong></h3>
        <ol>
          <li className='relative sm:p-4 py-3'>
            <input type="checkbox" className='acco opacity-0 absolute' name='accourding' id='cb1' />
            <label htmlFor="cb1" className='flex detail cursor-pointer bg-[#80808016] gap-3 rounded-sm mb-1 py-2 px-5 justify-between font-bold capitalize text-[1.0425em] leading-[1.35] tracking-[0.8px] backdrop-blur-[--blur]'>
              <p className='flex gap-3'><span>1.</span> Fresh Flavors, Friendly Prices</p></label>
            <p className='check-content shadow-xl hover:shadow-2xl max-h-[0px] overflow-hidden px-4 bg-[#80808016] sm:text-[1em] text-[0.95em] backdrop-blur-[--blur]'>
          ...
          <li className='relative sm:p-4 py-3'>
            ...
            <label htmlFor="cb2" className='flex detail cursor-pointer bg-[#80808016] gap-3 rounded-sm mb-1 py-2 px-5 justify-between font-bold capitalize text-[1.0425em] leading-[1.35] tracking-[0.8px] backdrop-blur-[--blur]'>
              <p className='flex gap-3'><span>2.</span>fast free wifi</p>
            </label>
            <p className='check-content shadow-xl hover:shadow-2xl max-h-[0px] overflow-hidden px-4 bg-[#80808016] sm:text-[1em] text-[0.95em] backdrop-blur-[--blur]'>
          ...
          <li className='relative sm:p-4 py-3'>
            ...
            <label htmlFor="cb3" className='flex detail cursor-pointer bg-[#80808016] gap-3 rounded-sm mb-1 py-2 px-5 justify-between font-bold capitalize text-[1.0425em] leading-[1.35] tracking-[0.8px] backdrop-blur-[--blur]'>
              <p className='flex gap-3'><span>3.</span>More Than Just a Meal</p>
            </label>
            <p className='check-content shadow-xl hover:shadow-2xl max-h-[0px] overflow-hidden px-4 bg-[#80808016] sm:text-[1em] text-[0.95em] backdrop-blur-[--blur]'>
          ...
          <li className='relative sm:p-4 py-3'>
            ...
            <label htmlFor="cb4" className='flex detail cursor-pointer bg-[#80808016] gap-3 rounded-sm mb-1 py-2 px-5 justify-between font-bold capitalize text-[1.0425em] leading-[1.35] tracking-[0.8px] backdrop-blur-[--blur]'>
              <p className='flex gap-3'><span>4.</span>Come for the Food, Stay for the Experience</p></label>
            <p className='check-content shadow-xl hover:shadow-2xl max-h-[0px] overflow-hidden px-4 bg-[#80808016] sm:text-[1em] text-[0.95em] backdrop-blur-[--blur]'>
...
function LoopSlide() {
  return (
    <div className="flex flex-nowrap gap-[1.875rem] pl-[1.875rem]">
      {
...
const Home = memo(() => {
  ...
  const viewportWidth = gsap.matchMedia()
  ...
  let specialItems = [];
  let arr = ['butter paneer masala', 'Daal baafle', 'malai kofta', 'kadai paneer', 'saahi paneer', 'paneer nudals', 'kheer']
  ...
  useGSAP((context) => {
    gsap.fromTo(infi.current.children, {
    ...
    gsap.fromTo(infi2.current.children, {
      transform: 'translate(300px,0px)'
    },
      {
        transform: 'translate(0px,0px)',
    ...
    gsap.utils.toArray('.homecardimg').forEach(element => {
      gsap.fromTo(element, { transform: "scale(1.125)" }, {
        transform: "scale(1)",
        position: 'relative',
        scrollTrigger: {
  ...
  let mealDivover = contextSafe((e) => {
    viewportWidth.add("(min-width:768px)", () => {

      // mealimg.current.style.zIndex = '-1'
      gsap.to(mealimg.current, { transform: 'rotate3d(0, 0, 1, -25deg)', left: e.nativeEvent.offsetX - 125 + 'px', duration: 1.2, ease: 'power3', translate: '0%' })
      ...
      timeout1.current = setTimeout(() => {
        gsap.to(mealimg.current, {
          left: '50%',
          transform: 'translate(-50%)', ease: 'power3',
          // left: '',
          // transform: '', ease: 'power3',
          duration: 1.5
        })
      }, 2000);
  ...
  return (
    <section className='homePage relative'>
      ...
      <div className='hero flex items-center overflow-x-hidden min-h-[90vh] relative p-[1.25rem]'>
        <div className="homecontent mx-auto py-14 px-[1.25rem] sm:w-[65%] lg:max-xl:w-[55%] xl:w-[45%] flex flex-col items-center gap-8">
          <div className='overflow-y-hidden'>
            <h1 className='relative uppercase max-sm:text-[2rem] max-sm:leading-[2.5rem] sm:text-4xl text-center'>Taste the different<span className='whitespace-nowrap'><span className='text-stroke inline-block [textShadow:1px_1px_1px_aliceblue,_4px_3px_0.125rem_black]' ref={pureveg}>pure veg </span> Now</span></h1>
          </div>
          ...
          <div className={`h-[15.625rem] relative self-stretch`} ref={mealDiv} onMouseMove={mealDivover}>
            <Link to='/menu' className='h-full w-full flex justify-center'>
              <img  rel="preload" fetchpriority="high" ref={mealimg} src={foodimg} alt="food img" className='md:absolute pointer-events-none translate-x-0 sm:h-[15.625rem] sm:w-[15.625rem] h-[14.625rem] w-[14.625rem] will-change-transform transform-gpu drop-shadow-[0_1.25rem_2.1875rem_rgba(0,0,0,63%)] dark:drop-shadow-[0_1.25rem_2.1875rem_rgba(255,255,255,25%)] sm:dark:drop-shadow-[0_1.25rem_2.1875rem_rgba(255,255,255,45%)]' />
            </Link>
          </div>

          <h3 className='text-center capitalize intro-line max-sm:text-[1.05rem] max-sm:leading-[1.5] sm:text-xl relative dark:text-white text-black'>Unforgettable flavors await, delicious dishes packed with flavor, <span className='intro-line-main tracking-[2px] [filter:drop-shadow(0_1px_1px_rgb(0_0_0))_drop-shadow(0_0px_0px_rgb(0_0_0))]' ref={linearline}>family-friendly atmosphere.</span> Gather, connect, and make memories! At your new favorite spot.
          </h3>

          {/* btn  */}
          <div className='flex flex-col items-center gap-4 md:max-lg:w-[60%] lg:max-xl:w-[55%] xl:w-1/2 min-w-[50%]'>
      ...
      {/* our special  */}
      <div className="page2 pt-[4.75rem] pb-[1.25rem]">
        <h3 className='max-sm:text-[1.15rem] text-xl pb-6 text-center [textShadow:0.125rem_1px_3px_#3b3b3b94]'>
          <span ref={ourspecial} className='capitalize font-semibold tracking-[0.5px] inline-block p-2 px-6 rounded text-center mx-auto origin-top shadow-xl will-change-auto'>our special</span>
        </h3>
        <h3 className='capitalize max-sm:text-[1.025rem] text-lg px-3 pb-6 text-center [text-shadow:2px_1px_0px_#00000035] font-medium leading-[normal] max-sm:max-w-[18.75rem] mx-auto'>A Taste of Tradition, A Touch of Innovation.</h3>

        {/*==== food cards  ====*/}
        <div className="flex p-[1.25rem] flex-wrap justify-center gap-y-[3.125rem] gap-5">
          {
            specialItems.map((menu, index) => (
              <div key={index} ref={homecard} className="homecard ar-one-sans relative grid overflow-hidden w-[18.75rem] min-h-[18.75rem] bg-[#00000012] border border-white shadow-[0_2.1875rem_3.75rem_-0.9375rem_rgba(0,0,0,0.3)] rounded-lg backdrop-blur-[--blur]">
                ...
                <div className="max-h-[11.25rem] mx-auto relative">
                  <img loading='lazy' ref={homecardsImg} src={menu.img} className='homecardimg w-full h-full object-contain will-change-auto' alt={menu.heading + ' image'} />
                </div>

                <div className="card-body p-[0.9375rem] self-end">
                  <h3 className="dark:text-white tracking-[.5px] text-slate-950 max-md:text-[0.925rem] font-medium capitalize">{menu.heading}</h3>
                  <h4 className="max-md:text-[0.905rem] font-semibold"> &#8377; {menu.price}</h4>
                </div>
                <div className="flex max-md:text-[0.825rem] text-[0.875rem] font-medium self-end">
                  {
                    menu.inCart ?
        ...
        <div className="py-[3.75rem] infi select-none">
          <div className="flex justify-end sm:py-5 py-3 flex-nowrap whitespace-nowrap overflow-hidden transform-cpu will-change-auto" ref={infi2}>
            <LoopSlide />
            <div className="flex flex-nowrap gap-[1.875rem] pl-[1.875rem]">
              {
                Array(5).fill('made with love').map((el, ind) => (
          ...
          <div className="flex sm:py-5 py-3 flex-nowrap whitespace-nowrap overflow-hidden transform-cpu will-change-auto" ref={infi}>
            <LoopSlide2 />
            <div className="flex flex-nowrap gap-[1.875rem] pl-[1.875rem]">
              {
                Array(5).fill('you will love every bite').map((el, ind) => (

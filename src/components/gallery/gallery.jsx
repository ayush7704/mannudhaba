...
function gallery() {
  let { PageHeading,WhatsAppLink } = useContext(globalContext)
  ...
  return (
    <section className='relative gallery backdrop-blur-[100px]'>
      ...
      <div className='sm:p-[1.25rem] py-6'>
        ...
        <div className="flex gap-4 md:justify-center p-[1.25rem_1.25rem] overflow-x-auto">
        ...
        <div className="md:flex grid gap-2 relative md:h-[90vh] md:mt-8 mt-4 items-start">
          <div ref={imgsList} className="md:w-[20%] p-2 md:h-full md:grid flex gap-2 overflow-y-auto">
            ...
            {filterArray.map((imgSrc, index) => (
              <div key={index} className="md:h-40 outLine flex-[0_0_9.375rem] h-20 rounded-md overflow-hidden">
                <img src={imgSrc.img} alt={`image-${index}`}
          ...
          <div className="md:w-[80%] sticky top-[--navHeight] max-md:h-[50vh] md:h-[80%] dark:border-white border-black border-1 rounded-md">
            {/* navigations start  */}
            ...
            {/* current img start  */}
            <img ref={currentImgRef} src={currentImg} alt="image" className={`h-full sm:min-h-[auto] min-h-[11.875rem] object-contain w-[100%]`} />
            {/* current img end */}
          </div>
        </div>
        ...
        <div className='flex-col flex justify-center gap-4 mt-2 p-5'>
          ...
          <WhatsAppLink classes='pb-3' number={7999741488} msg='Hello! Mannu Dhaba and Family Restaurant.'>
            <svg viewBox="0 0 24 24" className='self-center hover:text-[#25D366] dark:text-[#00ff5f] text-[#00cd4d] sm:w-[2.1875rem] sm:h-[2.1875rem] w-[2rem] h-[2rem] mx-auto' fill="none">
